...
async function runScraper() {
  ...
  broadcasts(input: $input) {
    ...
    // ===== THE HUB — RECURRING FREE MEALS — RETIRED 2026-07-28 =====
    // Replaced outright by the place-specials pipeline (replacement
    // precedent, like cupboardHoursObject): the `the-hub` entry in
    // place-specials.json (eventTag "HUB", three recurringEvents items,
    // JD-style activeRanges + expandCount 60/expandDays 365) emits Tue/Thu
    // Free Lunch + French Toast Friday via the specials assembly below.
    ...
    // (Community Board generator REMOVED 2026-07-28 — the Board page was
    // replaced by the Food page (full excision, replacement precedent). The
    // sheet fetch, per-category TTL policy, and board.json write are gone;
    // board.json is deleted from the repo. The Google Form + response sheet
    // (1FZ-eFzLYFAgNd7aBCrU5uwb5wMQ2x9tBf_KLGa6GJS0) can be closed at leisure.)
